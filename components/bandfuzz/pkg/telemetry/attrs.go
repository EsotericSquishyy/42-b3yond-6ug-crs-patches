...
func (o SpanAttributes) Attributes() []attribute.KeyValue {
	...
	if o.CodeFile.set {
		attrs = append(attrs, attribute.String("crs.action.code.file", o.CodeFile.val))
	}
	if o.CodeLines.set {
		attrs = append(attrs, attribute.String("crs.action.code.lines", o.CodeLines.val))
	}
	if o.TargetHarness.set {
		attrs = append(attrs, attribute.String("crs.action.target.harness", o.TargetHarness.val))
	}
	if o.EffortWeight.set {
		attrs = append(attrs, attribute.String("crs.action.effort.weight", o.EffortWeight.val))
	}
	...
	if o.corpusFullSnapShot.set {
		attrs = append(attrs, attribute.Bool("fuzz.corpus.full_snapshot", o.corpusFullSnapShot.val))
	}
