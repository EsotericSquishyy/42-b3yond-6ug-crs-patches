...
int FuzzerDriver(int *argc, char ***argv, UserCallback Callback) {
  ...
  // acd: generate hash for cmin
  if (Flags.generate_hash) {
    Printf("acd: generate all the coverage hash\n");
    // get all the inputs in the corpus
    UnitVector Corpus;
    std::vector<std::string> Files;
    for (auto &Path : *Inputs) {
      Printf("Loading corpus dir: %s\n", Path.c_str());
      ReadDirToVectorOfUnits(Path.c_str(), &Corpus, nullptr,
                             INT_MAX, /*ExitOnError=*/false);           
      ListFilesInDirRecursive(Path, 0, &Files, /*TopDir*/true);
    }
    // std::vector<uint64_t> Hashes;
    auto StartTime = system_clock::now();
    std::map<uint64_t, uint32_t> GlobalFeatureMap;
    std::set<uint64_t> GlobalFeatures;
    std::set<uint64_t> NewFeatures;
    
    for (size_t i = 0; i != Corpus.size(); ++i) {
      NewFeatures.clear();
      // Printf("acd: generating hash for %zd\n", i);
      if (sigsetjmp(JumpBuffer, 1) == 0) {
        auto &U = Corpus[i];

        auto Hash = F->RunAndHash(U.data(), U.size(), GlobalFeatures, NewFeatures);
        // auto &Path = Files[i];
        // Printf("%s\n", Path.c_str());
        for (auto &F : NewFeatures) {
          GlobalFeatureMap[F] = i;
        }
        // Hashes.push_back(Hash);
      } else {
        Printf("acd: Recoving from signal\n");
        continue;
      }
    }
    ...
    auto StopTime = system_clock::now();
    auto MS = duration_cast<milliseconds>(StopTime - StartTime).count();
    Printf("acd: generate cmin corpus by features in %ld ms\n", (long)MS);
    for (auto &F : GlobalFeatureMap) {
      Printf("clustercmin:%llu:%s\n", F.first, Files[F.second].c_str());
    }
    // for (size_t i = 0; i != Hashes.size(); ++i) {
    //   Printf("acd: %llx\n", Hashes[i]);
    // }
    Printf("acd: all the coverage hash generated\n");
    exit(0);
  }
