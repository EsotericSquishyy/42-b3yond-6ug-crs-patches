...
uint64_t Fuzzer::RunAndHash(const uint8_t *Data, size_t Size, std::set<uint64_t> &GlobalFeatures, std::set<uint64_t> &NewFeatures) {
  if (!Size)
    return 0;
  // Largest input length should be INT_MAX.
  assert(Size < std::numeric_limits<uint32_t>::max());
  ...
  if(!ExecuteCallback(Data, Size)) return 0;
  auto TimeOfUnit = duration_cast<microseconds>(UnitStopTime - UnitStartTime);
  ...
  TPC.CollectFeatures([&](uint32_t Feature) {
    if (GlobalFeatures.find(Feature) == GlobalFeatures.end()) {
      // If the feature is not in the global set, add it to the new features
      GlobalFeatures.insert(Feature);
      NewFeatures.insert(Feature);
    } 
      // UniqFeatureSetTmp.push_back(Feature);
      
      // Update hash by current hash and new feature
      // Hash = XXH64(&Feature, sizeof(Feature), Hash);
  });
  // TPC.UpdateObservedPCs();
  // TPC.ForEachObservedPC([&](const TracePC::PCTableEntry *TE) {
  //   // Printf("TE: %p\n", TE->PC);
  //   if (GlobalFeatures.find(TE->PC) == GlobalFeatures.end()) {
  //     GlobalFeatures.insert(TE->PC);
  //     NewFeatures.insert(TE->PC);
  //   }
  // });
  // TPC.ObservedPCs
  
  // calculate xxHash of features
  // uint64_t Hash = XXH64(UniqFeatureSetTmp.data(),
  //                       UniqFeatureSetTmp.size() * sizeof(UniqFeatureSetTmp[0]),
  //                       0); // Seed is 0
  
  
  // output hash
  // Printf("clustercmin:");
  // for (auto i : UniqFeatureSetTmp) {
  //   Printf("%u,", i);
  // }
  // Printf(":%llu:", Hash);
  // uint64_t Hash = 0;
