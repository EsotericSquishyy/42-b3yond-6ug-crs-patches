...
Please choose one of the file type (or protocol) in this list that you think best match the input that the fuzzing harness expects: {filetype_list}

You should return only the name of the file type (or protocol) in your response, and nothing else, e.g. `jpg`, `png`, `gif`, etc. or `http`, `tcp`, etc. The file type (or protocol) you respond with MUST be within the list given above. If the file type (or protocol) cannot not be determined (or not a commonly known one), you should return `unknown`.
