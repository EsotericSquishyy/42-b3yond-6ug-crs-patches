...
use argus::{
    ...
    option_visitors::{
        AdditionalObjectsVisitor, AdditionalPassesVisitor, CompilationDatabaseVisitor, DefaultOptimizationVisitor, DefaultParametersVisitor, LibfuzzerVisitor, OptionVisitor, ProfileVisitor, RuntimeVisitor, SanitizerVisitor, XVisitor
    },
};
...
fn main() {
    ...
    if std::env::var(GENERATE_COMPILATION_DATABASE).is_ok() {
        visitors.push(Box::<CompilationDatabaseVisitor>::default());
    }
