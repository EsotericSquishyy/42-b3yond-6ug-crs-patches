...
def span_decorator(name):
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            with create_span(f"Slice-14 {name}", parent_span=current_span) as span:
                try:
...
# Initialize tracer with service information
resource = Resource(attributes={
    "service.name": "slice-R14",
    "service.version": 1
})
...
try:
    ...
    if hasattr(provider, "add_span_processor"):
        ...
        logging.warning("Unable to add span processor to the current TracerProvider. Telemetry may be limited.")
except Exception as e:
...
def inject_span_context(carrier: Dict[str, str]) -> Dict[str, str]:
    """
    Inject current span context into a carrier dictionary.
    ...
    propagator.inject(carrier, context=span_ctx)
    return carrier
...
def get_telemetry_metric_mock() -> dict:
    ...
    return {
        ...
        "slice.corpus.update.method": "periodic",
        "title": [],
        "msg": [],
        "log.level": "verbose",
    }
...
def log_action(
    ...
    try:
        with tracer.start_as_current_span(crs_action_category) as span:
            ...
            span.set_attribute("title", slice_title)
            slice_msg_str = [str(msg) for msg in slice_msg]
...
def log_action_from_metrics(
    ...
    crs_action_category = metrics.get("crs.action.category", "slice")
    slice_title = metrics.get("title", [])
    slice_msg = metrics.get("msg", [])
    log_level = metrics.get("log.level", "verbose")

    status = StatusCode.OK if status_str == "OK" else StatusCode.ERROR

    log_action(slice_title, slice_msg, crs_action_category, crs_action_name, status, log_level)
